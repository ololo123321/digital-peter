...
class BeamEntry(ReprMixin):
    ...
    def __init__(self, alpha, beta):
        ...
        self.p_total = 0  # blank and non-blank
        self.p_non_blank = 0  # non-blank
        self.p_blank = 0  # blank
        self.p_lm = 1  # LM score
...
def test_beam_search():
    ...
    for beam in res.entries.values():
        assert 0 <= beam.p_lm <= 1, beam.p_lm
        assert 0 <= beam.p_blank <= 1, beam.p_blank
        assert 0 <= beam.p_non_blank <= 1, beam.p_non_blank
        assert 0 <= beam.p_total <= 1, beam.p_total
    print(res)
