...
def build_joint_model():
    ...
    lm_transformer_dir = "./lm/transformer"
    lm = models.TransformerLanguageModel.load(lm_transformer_dir)
    lm.build()
    lm.restore(lm_transformer_dir)
    ...
    return joint_decoder_inference, lm.char2id
...
def main():
    ...
    dec_model_new = tf.keras.Model(
        inputs=htr_model.dec_model.inputs,
        outputs=[
            htr_model.dec_model.get_layer("decoder_baseline").output,
            htr_model.dec_model.get_layer("dense").output
        ]
    )
    ...
    features_frames, char_prob = dec_model_new.predict(ds)
    ...
    joint_decoder_inference.load_weights("./joint/model_inference.hdf5")
