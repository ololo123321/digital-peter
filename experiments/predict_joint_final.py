...
from generators import JointIterator


# TODO: закомментить перед сабмитом
...
def build_joint_model(model_dir):
    tf.keras.backend.clear_session()

    # lm (нужна односторонняя)
    lm = models.TransformerLanguageModel.load(model_dir, config_name="config_lm.json")
    lm.build()
    char2id = json.load(open(os.path.join(model_dir, "char_encodings.json")))
    vocab_size_lm = len(char2id)

    # setup layers
    config_dec = json.load(open(os.path.join(model_dir, "config_decoder.json")))
...
def main():
    ...
    print('inference of the main htr model')
    ...
    print('inference of encoder of the first joint model')
    encoder = tf.keras.Model(
        inputs=htr_model.dec_model.inputs,
        outputs=htr_model.dec_model.get_layer("decoder_baseline").output
    )
    encoder.load_weights("./joint/0/encoder.hdf5")
    ds = htr_model.data_pipeline.build_test_dataset(examples, batch_size=128)
    features_frames_0 = encoder.predict(ds)

    print('inference of encoder of the second joint model')
    encoder.load_weights("./joint/1/encoder.hdf5")
    ds = htr_model.data_pipeline.build_test_dataset(examples, batch_size=128)
    features_frames_1 = encoder.predict(ds)
    ...
    print("birnn scoring first...")
    lm_birnn = build_and_restore(model_cls=models.BiRNNLanguageModelV2, model_dir="./lm/birnn/0")
    log_prob_birnn_0 = lm_birnn.predict(examples, batch_size=256)  # [num_examples * beam_width]
    log_prob_birnn_0 = log_prob_birnn_0.reshape((-1, BEAM_WIDTH))

    print("birnn scoring second...")
    lm_birnn = build_and_restore(model_cls=models.BiRNNLanguageModelV2, model_dir="./lm/birnn/1")
    log_prob_birnn_1 = lm_birnn.predict(examples, batch_size=256)  # [num_examples * beam_width]
    log_prob_birnn_1 = log_prob_birnn_1.reshape((-1, BEAM_WIDTH))

    print("birnn blending...")
    log_prob_birnn = log_prob_birnn_0 * 0.5 + log_prob_birnn_1 * 0.5

    print("attn scoring first...")
    joint_decoder_inference, char2id = build_joint_model(model_dir='./joint/0')
    joint_decoder_inference.load_weights("./joint/0/decoder.hdf5")
    log_prob_joint_0 = predict_fast(
        decoder=joint_decoder_inference,
        examples=examples,
        features_frames=features_frames_0,
        char2id=char2id,
        img2id=img2id,
        batch_size=128
    )
    log_prob_joint_0 = log_prob_joint_0.reshape((-1, BEAM_WIDTH))

    print("attn scoring second...")
    joint_decoder_inference, char2id = build_joint_model(model_dir='./joint/1')
    joint_decoder_inference.load_weights("./joint/1/decoder.hdf5")
    log_prob_joint_1 = predict_fast(
        decoder=joint_decoder_inference,
        examples=examples,
        features_frames=features_frames_1,
        char2id=char2id,
        img2id=img2id,
        batch_size=128
    )
    log_prob_joint_1 = log_prob_joint_1.reshape((-1, BEAM_WIDTH))

    print("joint blending...")
    log_prob_joint = log_prob_joint_0 * 0.5 + log_prob_joint_1 * 0.5

    # weighting
    print("weighting...")
    scores = log_prob_ctc * W_CTC + log_prob_birnn * W_BIRNN + log_prob_joint * W_JOINT  # [num_examples, beam_width]
